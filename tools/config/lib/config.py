...
class Config:
    ...
    def __init__(self, lists:dict) -> None:
        ...
        for name, config_item in self.lists.items():
            if not 'depends' in config_item:
                continue
            dependencies = config_item['depends']
            for dependency in dependencies:
                dependency_config_item = self.lists.get(dependency)
                if dependency_config_item is None:
                    raise ConfigErr(f'Config {name} has an unknown dependency config f{dependency}')
